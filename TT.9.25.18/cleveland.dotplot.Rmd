...
We're also going to go ahead & drop our rank columns because we won't be needing them for the final plot. Finally, we're going to use gather() to make our data frame "longer" & easier to plot.
...
Looks good so far! First, let's change up the colors & make the points bigger. We can use "size =" in our geom_point() call to change the size of our dots. Make sure to put it outside of the aes() call.
...
Thanks to [othonamtegazza](https://github.com/othomantegazza) for the excellent feedback!
