...
## Subset data by year (2016) & country (USA)
datan<-subset(data, year == 2016) %>% subset(countrycode == "USA")
...
A<-ggplot(dfm, aes(reorder(variable,value), value, fill=variable)) +
  ...
  geom_hline(yintercept=c(5,10,15,20,25,30), linetype="dotted", color="gray") +
  ylab("") +
  xlab("") +
  scale_fill_manual(values = getPalette(colourCount)) +
  ggtitle("Share of deaths by cause, United States, 2016")

## Save to file
png("US.2016.png", height=500, width=600)
plot(A)
dev.off()
